...
pub(crate) fn routing_table(
    ...
    let seg = if !location.is_empty() && special_locs.iter().any(|s| s.as_str() == location) {
...
pub(crate) fn transform(
    ...
    let rule_groups: Vec<&str> = rule
        .get("groups")
        .and_then(Value::as_array)
        .map(|a| a.iter().filter_map(Value::as_str).collect())
        .unwrap_or_default();
    let finding_types = serde_json::to_string(&rule_groups).unwrap_or_else(|_| "[]".into());
        ...
        if !cve.is_empty() {
            ...
            let score_str = {
                ...
                if !s.is_empty() {
                    ...
                    get_data_field(&data_val, "vulnerability.score.base")
                }
            };
            cvss_score = score_str.parse::<f32>().unwrap_or(0.0);
            // Backfill existing typed columns if not already set
            if url.is_empty() {
                url = jpath(&data_val, "vulnerability.reference").to_string();
            }
            if app_name.is_empty() {
                app_name = jpath(&data_val, "vulnerability.package.name").to_string();
            }
            if status.is_empty() {
                status = jpath(&data_val, "vulnerability.status").to_string();
            }
        }
    }
    ...
    // Natively extracted — no field_mappings.toml entry required.
    if app_name.is_empty() {
        let v = jpath(&data_val, "win.system.providerName");
        if !v.is_empty() {
            app_name = v.to_string();
        }
    }
    if process_id == 0 {
        let v = jpath(&data_val, "win.system.processID");
        if !v.is_empty() {
            process_id = v.parse().unwrap_or(0);
        }
    }
    if process_name.is_empty() {
        let v = jpath(&data_val, "win.eventdata.processName");
        if !v.is_empty() {
            process_name = v.to_string();
        }
    }
    if actor_user.is_empty() {
        ...
        let v = if v.is_empty() {
            jpath(&data_val, "win.eventdata.subjectUserSid")
        } else {
            v
        };
        if !v.is_empty() {
            actor_user = v.to_string();
        }
    }
    if target_user.is_empty() {
        let v = jpath(&data_val, "win.eventdata.targetUserName");
        let v = if v.is_empty() {
            jpath(&data_val, "win.eventdata.targetUserSid")
        } else {
            v
        };
        if !v.is_empty() {
            target_user = v.to_string();
        }
    }
    ...
    // Natively extracted — no field_mappings.toml entry required.
    if app_name.is_empty() {
        if let Some(s) = data_val.get("package").and_then(Value::as_str) {
            if !s.is_empty() {
    ...
    if status.is_empty() {
        if let Some(s) = data_val.get("dpkg_status").and_then(Value::as_str) {
            if !s.is_empty() {
                status = s.to_string();
            }
        }
    ...
    // Compliance Finding rows have meaningful typed values, not just raw_data.
    {
        ...
        if finding_title_override.is_empty() {
            ...
            if !v.is_empty() {
                finding_title_override = v.to_string();
            }
        }
    ...
    {
        let v = jpath(&data_val, "win.system.eventID");
        if !v.is_empty() {
            extensions.insert("win_event_id".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "win.system.channel");
        if !v.is_empty() {
            extensions.insert("win_channel".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "win.eventdata.logonType");
        if !v.is_empty() {
            extensions.insert("win_logon_type".into(), Value::String(v.to_string()));
        }
    }
    ...
    for i in 1u8..=7 {
        ...
        if !v.is_empty() {
            extensions.insert(format!("win_param{i}"), Value::String(v.to_string()));
        }
    }
    // Windows raw event binary (hex-encoded)
    {
        let v = jpath(&data_val, "win.eventdata.binary");
        if !v.is_empty() {
            extensions.insert("win_event_binary".into(), Value::String(v.to_string()));
        }
    }
    // SCA extended context (check details beyond the typed columns above)
    {
        let v = jpath(&data_val, "sca.scan_id");
        if !v.is_empty() {
            extensions.insert("sca_scan_id".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.id");
        if !v.is_empty() {
            extensions.insert("sca_check_id".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.cis");
        if !v.is_empty() {
            extensions.insert("sca_cis_control".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.cis_csc");
        if !v.is_empty() {
            extensions.insert("sca_cis_csc".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.description");
        if !v.is_empty() {
            extensions.insert("sca_description".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.rationale");
        if !v.is_empty() {
            extensions.insert("sca_rationale".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.remediation");
        if !v.is_empty() {
            extensions.insert("sca_remediation".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.reason");
        if !v.is_empty() {
            extensions.insert("sca_reason".into(), Value::String(v.to_string()));
        }
    }
    // SCA extended compliance framework tags (multiple standards observed in the field)
    {
        let v = jpath(&data_val, "sca.check.compliance.cis_csc_v7");
        if !v.is_empty() {
            extensions.insert("sca_cis_csc_v7".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.cis_csc_v8");
        if !v.is_empty() {
            extensions.insert("sca_cis_csc_v8".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.cmmc_v2.0");
        if !v.is_empty() {
            extensions.insert("sca_cmmc_v2".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.hipaa");
        if !v.is_empty() {
            extensions.insert("sca_hipaa".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.iso_27001-2013");
        if !v.is_empty() {
            extensions.insert("sca_iso_27001".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.mitre_mitigations");
        if !v.is_empty() {
            extensions.insert("sca_mitre_mitigations".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.mitre_tactics");
        if !v.is_empty() {
            extensions.insert("sca_mitre_tactics".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.mitre_techniques");
        if !v.is_empty() {
            extensions.insert("sca_mitre_techniques".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.nist_sp_800-53");
        if !v.is_empty() {
            extensions.insert("sca_nist_800_53".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.pci_dss_v3.2.1");
        if !v.is_empty() {
            extensions.insert("sca_pci_dss_v3".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.pci_dss_v4.0");
        if !v.is_empty() {
            extensions.insert("sca_pci_dss_v4".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.compliance.soc_2");
        if !v.is_empty() {
            extensions.insert("sca_soc2".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.command.0");
        if !v.is_empty() {
            extensions.insert("sca_check_command".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.file.0");
        if !v.is_empty() {
            extensions.insert("sca_check_file".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.check.references");
        if !v.is_empty() {
            extensions.insert("sca_references".into(), Value::String(v.to_string()));
        }
    }
    // SCA policy-level summary fields (scan result overview rows)
    {
        let v = jpath(&data_val, "sca.policy_id");
        if !v.is_empty() {
            extensions.insert("sca_policy_id".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.description");
        if !v.is_empty() {
            extensions.insert(
                "sca_policy_description".into(),
                Value::String(v.to_string()),
            );
        }
    }
    {
        let v = jpath(&data_val, "sca.file");
        if !v.is_empty() {
            extensions.insert("sca_policy_file".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.score");
        if !v.is_empty() {
            extensions.insert("sca_score".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.total_checks");
        if !v.is_empty() {
            extensions.insert("sca_total_checks".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.passed");
        if !v.is_empty() {
            extensions.insert("sca_passed_count".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.failed");
        if !v.is_empty() {
            extensions.insert("sca_failed_count".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "sca.invalid");
        if !v.is_empty() {
            extensions.insert("sca_invalid_count".into(), Value::String(v.to_string()));
        }
    }
    // Linux audit AVC context
    {
        let v = jpath(&data_val, "audit.type");
        if !v.is_empty() {
            extensions.insert("audit_type".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "audit.id");
        if !v.is_empty() {
            extensions.insert("audit_id".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "audit.euid");
        if !v.is_empty() {
            extensions.insert("audit_euid".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "audit.uid");
        if !v.is_empty() {
            extensions.insert("audit_uid".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "audit.gid");
        if !v.is_empty() {
            extensions.insert("audit_gid".into(), Value::String(v.to_string()));
        }
    }
    {
        let v = jpath(&data_val, "audit.session");
        if !v.is_empty() {
            extensions.insert("audit_session".into(), Value::String(v.to_string()));
        }
    }
    // FIM / syscheck hash digests
    {
        let v = get_sc("md5_after");
        if !v.is_empty() {
            extensions.insert("fim_md5".into(), Value::String(v));
        }
    }
    {
        let v = get_sc("sha1_after");
        if !v.is_empty() {
            extensions.insert("fim_sha1".into(), Value::String(v));
        }
    }
    {
        let v = get_sc("sha256_after");
        if !v.is_empty() {
            extensions.insert("fim_sha256".into(), Value::String(v));
        }
    }
    {
        let v = get_sc("size_after");
        if !v.is_empty() {
            extensions.insert("fim_size".into(), Value::String(v));
        }
    }
    {
        let v = get_sc("mode");
        if !v.is_empty() {
            extensions.insert("fim_mode".into(), Value::String(v));
        }
    }
    if let Some(ca) = syscheck_obj.get("changed_attributes") {
        if let Ok(s) = serde_json::to_string(ca) {
            extensions.insert("fim_changed_attrs".into(), Value::String(s));
        }
    }
    // dpkg package version + arch
    if let Some(s) = data_val.get("version").and_then(Value::as_str) {
        if !s.is_empty() {
            extensions.insert("package_version".into(), Value::String(s.to_string()));
        }
    }
    if let Some(s) = data_val.get("arch").and_then(Value::as_str) {
        if !s.is_empty() {
            extensions.insert("package_arch".into(), Value::String(s.to_string()));
        }
    }
    // Process / sudo context
    if let Some(s) = data_val.get("uid").and_then(Value::as_str) {
        if !s.is_empty() {
            extensions.insert("actor_uid".into(), Value::String(s.to_string()));
        }
    }
    if let Some(s) = data_val.get("tty").and_then(Value::as_str) {
        if !s.is_empty() {
            extensions.insert("tty".into(), Value::String(s.to_string()));
        }
    }
    if let Some(s) = data_val.get("pwd").and_then(Value::as_str) {
        if !s.is_empty() {
            extensions.insert("working_dir".into(), Value::String(s.to_string()));
        }
    }
    // Other generic decoder context fields
    if file_name.is_empty() {
        if let Some(s) = data_val.get("file").and_then(Value::as_str) {
            if !s.is_empty() {
    ...
    if finding_title_override.is_empty() {
        if let Some(s) = data_val.get("title").and_then(Value::as_str) {
            if !s.is_empty() {
                finding_title_override = s.to_string();
            }
        }
    }
    if let Some(s) = data_val.get("gid").and_then(Value::as_str) {
        if !s.is_empty() {
            extensions.insert("actor_gid".into(), Value::String(s.to_string()));
        }
    }
    if let Some(s) = data_val.get("home").and_then(Value::as_str) {
        if !s.is_empty() {
            extensions.insert("actor_home_dir".into(), Value::String(s.to_string()));
        }
    }
    if let Some(s) = data_val.get("shell").and_then(Value::as_str) {
        if !s.is_empty() {
            extensions.insert("actor_shell".into(), Value::String(s.to_string()));
        }
    }
    ...
    if let Some(aws) = data_val.get("aws").and_then(Value::as_object) {
        ...
        macro_rules! aws_ext {
            ($key:expr, $ext:expr) => {
                ...
                if !_v.is_empty() {
                    extensions.insert($ext.into(), Value::String(_v.to_string()));
                }
            };
        }
        ...
        if let Some(uid) = aws.get("userIdentity").and_then(Value::as_object) {
            ...
            if let Some(sc) = uid.get("sessionContext").and_then(Value::as_object) {
                ...
                if let Some(si) = sc.get("sessionIssuer").and_then(Value::as_object) {
                    let v = aws_str!(si, "type");
                    if !v.is_empty() {
                        extensions.insert(
                            "aws_session_issuer_type".into(),
                            Value::String(v.to_string()),
                        );
                    }
                    let v = aws_str!(si, "principalId");
                    if !v.is_empty() {
                        extensions.insert(
                            "aws_session_principal_id".into(),
                            Value::String(v.to_string()),
                        );
                    }
                    let v = aws_str!(si, "accountId");
                    if !v.is_empty() {
                        extensions.insert(
                            "aws_session_issuer_account".into(),
                            Value::String(v.to_string()),
                        );
                    }
                }
                if let Some(attrs) = sc.get("attributes").and_then(Value::as_object) {
                    let v = aws_str!(attrs, "mfaAuthenticated");
                    if !v.is_empty() {
                        extensions
                            .insert("aws_session_mfa_auth".into(), Value::String(v.to_string()));
                    }
                    let v = aws_str!(attrs, "creationDate");
                    if !v.is_empty() {
                        extensions.insert(
                            "aws_session_created_at".into(),
                            Value::String(v.to_string()),
                        );
                    }
                }
                if let Some(wif) = sc.get("webIdFederationData").and_then(Value::as_object) {
                    let v = aws_str!(wif, "federatedProvider");
                    if !v.is_empty() {
                        extensions.insert(
                            "aws_federated_provider".into(),
                            Value::String(v.to_string()),
                        );
                    }
                }
        ...
        if let Some(rp) = aws.get("requestParameters").and_then(Value::as_object) {
            ...
            if !v.is_empty() {
                extensions.insert("aws_kms_key_id".into(), Value::String(v.to_string()));
            }
            let v = aws_str!(rp, "networkInterfaceId");
            if !v.is_empty() {
                extensions.insert(
                    "aws_req_network_interface_id".into(),
                    Value::String(v.to_string()),
                );
            }
            let v = aws_str!(rp, "groupId");
            if !v.is_empty() {
                extensions.insert(
                    "aws_req_security_group_id".into(),
                    Value::String(v.to_string()),
                );
            }
            let v = aws_str!(rp, "subnetId");
            if !v.is_empty() {
                extensions.insert("aws_req_subnet_id".into(), Value::String(v.to_string()));
            }
            let v = aws_str!(rp, "snapshotId");
            if !v.is_empty() {
                extensions.insert("aws_req_snapshot_id".into(), Value::String(v.to_string()));
            }
            let v = aws_str!(rp, "volumeId");
            if !v.is_empty() {
                extensions.insert("aws_req_volume_id".into(), Value::String(v.to_string()));
            }
            let v = aws_str!(rp, "allocationId");
            if !v.is_empty() {
                extensions.insert("aws_req_allocation_id".into(), Value::String(v.to_string()));
            }
        }

        // responseElements sub-fields (EC2 resource creation / modification results)
        if let Some(re) = aws.get("responseElements").and_then(Value::as_object) {
            let v = aws_str!(re, "publicIp");
            if !v.is_empty() {
                extensions.insert("aws_res_public_ip".into(), Value::String(v.to_string()));
            }
            let v = aws_str!(re, "networkInterfaceId");
            if !v.is_empty() {
                extensions.insert(
                    "aws_res_network_interface_id".into(),
                    Value::String(v.to_string()),
                );
            }
            let v = aws_str!(re, "allocationId");
            if !v.is_empty() {
                extensions.insert("aws_res_allocation_id".into(), Value::String(v.to_string()));
            }
            let v = aws_str!(re, "snapshotId");
            if !v.is_empty() {
                extensions.insert("aws_res_snapshot_id".into(), Value::String(v.to_string()));
            }
            let v = aws_str!(re, "volumeId");
            if !v.is_empty() {
                extensions.insert("aws_res_volume_id".into(), Value::String(v.to_string()));
            }
        }
        ...
        if let Some(li) = aws.get("log_info").and_then(Value::as_object) {
            let v = aws_str!(li, "log_file");
            if !v.is_empty() {
                extensions.insert("aws_log_file".into(), Value::String(v.to_string()));
            }
            let v = aws_str!(li, "s3bucket");
            if !v.is_empty() {
                extensions.insert("aws_log_s3bucket".into(), Value::String(v.to_string()));
            }
        }
        ...
        match aws_source {
            ...
            "guardduty" | "inspector" | "macie" => {
                ...
                if aws_source == "guardduty" {
                    ...
                    let sev_raw =
                        if let Some(sev_obj) = aws.get("severity").and_then(Value::as_object) {
                            sev_obj
                                .get("description")
                                .and_then(Value::as_str)
                                .unwrap_or("")
            ...
            "config" => {
                ...
                aws_ext!("configuration.complianceType", "aws_compliance_type");
                {
                    ...
                    if !v.is_empty() {
                        extensions
                            .insert("aws_config_rule_name".into(), Value::String(v.to_string()));
                    }
                }
            }
    ...
    if let Some(gcp) = data_val.get("gcp").and_then(Value::as_object) {
        macro_rules! gcp_str {
            ($k:expr) => {
                gcp.get($k).and_then(Value::as_str).unwrap_or("")
            };
        }
        macro_rules! gcp_ext {
            ($k:expr, $ext:expr) => {
    ...
    if let Some(docker) = data_val.get("docker").and_then(Value::as_object) {
        macro_rules! dk_str {
            ($k:expr) => {
                docker.get($k).and_then(Value::as_str).unwrap_or("")
            };
        }
            ...
            .and_then(Value::as_object)
        {
            if let Some(v) = attr.get("role.new").and_then(Value::as_str) {
                if !v.is_empty() {
                    extensions.insert("docker_role_new".into(), Value::String(v.to_string()));
                }
            }
            if let Some(v) = attr.get("role.old").and_then(Value::as_str) {
                if !v.is_empty() {
                    extensions.insert("docker_role_old".into(), Value::String(v.to_string()));
                }
            }
            if let Some(v) = attr.get("image").and_then(Value::as_str) {
                if !v.is_empty() {
                    extensions.insert("docker_image".into(), Value::String(v.to_string()));
                }
            }
        }
    }
    ...
    if let Some(msg) = data_val.get("ms-graph").and_then(Value::as_object) {
        macro_rules! msg_str {
            ($k:expr) => {
                msg.get($k).and_then(Value::as_str).unwrap_or("")
            };
        }
        macro_rules! msg_ext {
            ($k:expr, $ext:expr) => {
    ...
    if !custom.field_map.is_empty() {
        for (wazuh_field, ocsf_target) in &custom.field_map {
            ...
            match ocsf_target.as_str() {
                ...
                "http_status" => {
                    if http_status == 0 {
                        http_status = val.parse().unwrap_or(0);
                    }
                }
                "app_name" => {
                    if app_name.is_empty() {
                ...
                "process_id" => {
                    if process_id == 0 {
                        process_id = val.parse().unwrap_or(0);
                    }
                }
                "rule_name" => {
                    if rule_name.is_empty() {
                ...
                "nat_src_ip" => {
                    if nat_src_ip.is_empty() {
                        nat_src_ip = val;
                    }
                }
                "nat_dst_ip" => {
                    if nat_dst_ip.is_empty() {
                        nat_dst_ip = val;
                    }
                }
                "nat_src_port" => {
                    if nat_src_port == 0 {
                        nat_src_port = val.parse().unwrap_or(0);
                    }
                }
                "nat_dst_port" => {
                    if nat_dst_port == 0 {
                        nat_dst_port = val.parse().unwrap_or(0);
                    }
                }
                "dst_hostname" => {
                    if dst_hostname.is_empty() {
                ...
                "bytes_in" => {
                    if bytes_in == 0 {
                        bytes_in = val.parse().unwrap_or(0);
                    }
                }
                "bytes_out" => {
                    if bytes_out == 0 {
                        bytes_out = val.parse().unwrap_or(0);
                    }
                }
                "network_protocol" => {
                    if network_protocol.is_empty() {
                        network_protocol = val;
                    }
                }
                other => {
                    extensions.insert(other.to_string(), Value::String(val));
                }
            }
        }
    }
    let extensions_json = serde_json::to_string(&extensions).unwrap_or_else(|_| "{}".into());

    // ── Full data sub-object (lossless) ───────────────────────────────────
    let event_data = serde_json::to_string(&data_val).unwrap_or_else(|_| "{}".into());
    ...
    let unmapped_obj: Map<String, Value> = obj
        .iter()
        .filter(|(k, _)| !KNOWN.contains(&k.as_str()))
        .map(|(k, v)| (k.clone(), v.clone()))
        .collect();
    let unmapped = serde_json::to_string(&unmapped_obj).unwrap_or_else(|_| "{}".into());
    ...
    if status.is_empty() {
        if rule_groups.iter().any(|&g| {
            matches!(
                g,
                "authentication_success" | "win_authentication_success" | "pam_success" | "sudo"
            )
        }) {
            status = "Success".to_string();
        } else if rule_groups.iter().any(|&g| {
            matches!(
                g,
    ...
    let (activity_id, activity_name): (u8, &str) = match ocsf_cls.class_uid {
        ...
        1006 => {
            if grp("process_stopped")
                || grp("process_terminated")
        ...
        3001 => {
            if grp("userdel") {
                (2, "Delete User")
            } else if grp("groupdel") {
                (8, "Delete Group")
            } else if grp("addgroup") {
                (7, "Create Group")
            } else if grp("groupmod") {
                (9, "Update Group")
            } else if grp("usermod") || grp("account_changed") {
                (3, "Update User")
            } else if grp("passwd") || grp("password_changed") {
                (12, "Change Password")
            } else {
                (1, "Create User")
            }
        ...
        4001 => match action.to_ascii_lowercase().as_str() {
            "allow" | "allowed" | "permit" | "pass" | "accept" | "accepted" | "open" => (1, "Open"),
            "deny" | "denied" | "block" | "blocked" | "drop" | "dropped" | "reject"
            | "rejected" | "refuse" | "refused" => (5, "Refuse"),
            "close" | "closed" | "end" | "finish" | "finished" | "timeout" | "timed_out" => {
                (2, "Close")
            }
            "reset" | "rst" => (3, "Reset"),
    ...
    let status_id: u8 = match ocsf_cls.class_uid {
        ...
        3002 => match status.to_ascii_lowercase().as_str() {
            ...
            "failure" | "fail" | "failed" | "deny" | "denied" | "block" | "blocked" | "drop"
            | "dropped" | "reject" | "rejected" | "error" => 2,
            "" => 0,
            _ => 99,
        },
    ...
    let rec = OcsfRecord {
        ...
        rule_name,
        app_category,
        finding_title: if finding_title_override.is_empty() {
            rule_desc
        } else {
